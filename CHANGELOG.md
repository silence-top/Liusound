...
产品版本号以 `pubspec.yaml` 的 `version` 为唯一事实来源。变更按主题分节，架构侧详情见 `FEATURES.md`（§13 Invariants / §14 Anti-Patterns / §15 P1 整改补充 / §16 元数据插件）。

## 2026-09-20 — 元数据插件系统（外部头像/相似歌曲/歌手简介）

### 新功能

- **插件化取数**：歌手头像、相似歌曲、歌手简介支持接入外部数据源插件。插件为**纯声明式 JSON 描述**（HTTP 端点 + 响应提取路径 + 可选 key 注入），不含可执行代码，所有端点强制 https；官方模板与用户导入的第三方描述走同一条执行管道（受 DeepSeek Cordis「声明式组件加载」思路启发，架构详见 FEATURES.md §十六）
- **三个官方插件**（随包内置，`assets/plugins/`，均免 key）：
  - **Deezer**：歌手头像 + 相似歌手；国内直连可用，华语歌手覆盖良好
  - **TheAudioDB**：歌手头像 + 简介（搜索取 id → 详情端点两步流，中文简介优先、英文兜底）；国际歌手中文简介覆盖良好
  - **百度百科**：华语歌手中文简介（abstract，400 字级；headers 声明 UA/Referer 后实测稳定），补齐国内简介缺口
  - Last.fm 原为预设之一，实测 `ws.audioscrobbler.com` 国内直连 403 而移出；自备代理+key 可按 schema 自行导入
- **提取路径增强**：`|` 分隔备选路径（取首个非 null）；简介能力节支持两步流（`idPath` + `fetchUrl`，与相似歌手同构）
- **设置 → 元数据插件**：官方插件一键启停、key 管理、导入第三方 JSON 描述（file_picker）、已导入插件启停/删除；主页入口副标题实时显示生效能力（头像/相似歌曲/歌手简介）
- **取数编排**：按注册表顺序（官方 → 导入）取第一个有结果者；`required` key 未填的插件直接短路不打请求；全局开关关闭时不发起任何外部请求；插件请求走用户网络设置（代理/超时/证书）
- **相似歌曲编排**：服务端原生相似歌曲为空或后端不支持时，插件取相似歌手名 → 按名字映射回本地曲库 → 取每位歌手热门曲（上限 12 首），结果全部本地可播；映射不上的歌手自然不出现在结果中
- **歌手简介编排**：服务端简介（getArtistInfo）为空或后端无此能力时走插件兜底
- **歌手头像编排**：服务端无图（hasCover=false 或封面加载失败）时优先插件外部头像、其次专辑封面兜底；歌手/专辑艺术家列表行、搜索结果行、歌手歌曲页头部生效
- **能力派生解锁**：任一相关插件可用即解锁播放页「相似歌曲/歌手简介」分区——Jellyfin/Emby/Plex/Audio Station/fnos 等原生缺失的后端同步受益
- **结果缓存**：按歌手名持久缓存（头像 URL 30 天 / 简介 7 天 / 无结果负缓存 7 天），复访零外部请求；头像图片本体复用封面专用磁盘缓存
...
## 2026-09-20 — 播放页 Tab 选中态改下划线指示条

- 播放页顶部三 Tab（推荐/歌曲/歌词）弃用白底胶囊选中态：大色块与封面取色渐变背景不协调，且此前实现意外撑满整行与下方内容贴死
- 改为文字强调（选中加粗提亮）+ 底部 20×3 圆角短横线，滑动切页时指示条连续滑行、文字颜色渐变，无底色块干扰封面氛围

## 2026-09-20 — 修复转码流首播进度条 0:00

- **问题**：服务端转码响应是分块流（无 Content-Length），播放器解不出总时长——首次播放未播过的歌进度条恒 0:00；重放时边听边存已有完整缓存文件可解出时长，故正常
- **修复**：① Subsonic 系（Navidrome/Subsonic）转码请求追加 `estimateContentLength=true`（Subsonic API 标准参数，服务端按目标码率估算长度返回 Content-Length，播放器可解出时长且可拖动进度；不支持的服务端忽略之，无害）；② 播放总时长流增加兜底——播放器报不出时长时回退当前歌的服务端元数据时长，迷你条与播放页进度条立即有总长度

## 2026-09-20 — 相似歌曲提速（缓存 + 并行）

- **相似歌手名落盘缓存**：Deezer 相似歌手名此前完全没有缓存（头像/简介均有），每次重进推荐页都重新搜索+related 连打外部源；现与简介同策略落盘 7 天（含空名单负缓存），二次进入零外部请求
- **会话内按歌曲 LRU 复用**：相似歌曲结果按歌曲 id 记忆（上限 32，切歌释放 provider 后重进同曲即时回显）；只记非空结果，插件开关刚切换时不会被空结果钉死
- **本地取歌并行化**：命中多个本地歌手时此前逐个串行请求自家服务器，现并行取回（上限 8 人 × 3 首，凑 12 首止）

## 2026-09-20 — 相似歌曲出数修复（多候选回退 + 繁体拼音匹配）

- **多候选回退**：插件执行器 similar 两步流此前只认搜索首个候选 id，遇到重名占位条目（如 Deezer 的 陶喆/张学友 首条 related 为空）直接空手而归；现把 idPath 数字下标换成 `*` 通配取全部候选 id，依次尝试（至多 3 次）直到 related 出数
- **繁体拼音匹配**：相似歌手名→本地曲库映射增加拼音键兜底——外部源常给繁体变体（林俊傑/薛之謙/張惠妹），lpinyin 拼音归一后可与本地简体名对上（罗马化写法如 "Jay Chou" 仍对不上，属数据源固有缺口）

## 2026-09-20 — 首页封面卡标题单行化 + 每日推荐行播放图标回归

- **封面卡标题单行**：最新专辑／最近播放／最常播放横向封面卡标题从两行收敛为一行，放不下时自动往返扫动一遍读全标题（单次有界动画、不无限循环，省电模式保持省略号），卡片高度相应收紧
- **每日推荐行播放图标回归**：恢复每日推荐歌曲行尾的播放按钮（上一批次改横向卡时误删）
- 跑马灯组件 `MarqueeText` 新增 `autoScroll` 单次扫动模式（此前组件无调用方），手动长按滚动模式保留

## 2026-09-20 — 首页横向封面与性能优化

- **首页最近／最常播放**：复用最新专辑同款横向封面卡片，保留标签切换、点歌、长按操作与查看全部；两组横向滚动位置独立保存，其他界面视觉不变。
- **首页刷新**：复用在途请求并保留当前内容，空列表快照也立即回放；旧请求成功或失败均不能覆盖切服／重建后的状态；内容不变时跳过通知与重复写盘。
- **封面缓存补正**：明确设置 4000 项及 365 天闲置清理期，修正此前只有说明、实际仍用默认容量的遗漏；缓存请求调度也限制为 8 并发。
- **取色减负**：原生端取色与显示共用 300 档磁盘缓存及下载去重，实际解码限制在 64×64 内；成功颜色短期复用，减少反复切歌的计算。Web 使用浏览器网络图片缓存。
- **大队列与全库刷新**：随机队列维护改为线性合并；同一服务器／适配器／数据类型的在途全库同步合并，避免重复拉取和写库。

## 2026-09-20 — 封面加载与缓存链路修复

### 性能

- **封面磁盘缓存扩容**：所有网络封面（歌曲/歌手/专辑）从 `DefaultCacheManager`（默认仅 200 个文件 / 30 天，资料库封面几百张导致 LRU 不断驱逐、反复重下）切换到专用 `CoverCacheManager`：4000 项 / 365 天，缓存不再频繁轮转
- **封面请求并发限 8 + 15s 连接超时**：限制缓存请求调度和原生每主机连接数，减少列表集中加载的竞争；15s 约束建立连接的等待，不代表整个响应的超时或自动重试
- **统一封面尺寸档位**：歌曲弹层（原 104）与详情页头部（原 180）改为 300 档，与列表页同 URL 共享磁盘缓存，同一张封面不再因 URL 变体重复下载
- **切歌取色共享缓存**：原生端复用 300 档封面的缓存和在途下载；命中有效缓存无需网络，缺失时下载并入缓存，过期时沿用缓存管理器的更新策略
- **「清理图片缓存」同步清空封面专用缓存**

## 2026-09-19 — 首页/队列/顶栏视觉打磨 + 主题选择器回退

### 播放器

- **播放队列对齐播放页材质**：头部「清空 / 顺序播放」两枚按钮改播放页弹层同款白色半透明胶囊（白 0.08 底，不随封面取色），高 36 紧凑 shrinkWrap，与标题同行两端对齐；当前播放行高亮改白色半透明圆角衬底，行前景统一 onSurface，序号列即拖拽把手（32 宽），封面 40
- **顶栏 Tab 图标化**：壳层底部导航首页/资料库/设置由文字改为轮廓/实心图标切换（保留语义 label 与下划线指示器）
- **首页装饰收敛**：私人 FM 入口收为 32 图标色块；歌曲卡片行删除尾部冗余播放键；区块入口箭头统一 dim 色

### 设置

- **主题选择器回退旧版形态**：外观页主题项恢复「设置行点击展开双列迷你预览卡」交互（上一版常开大缩略图卡废弃）；预览卡仍用各皮肤自身 token 绘制底色/面板/描边/发光/主题色点

### 皮肤

- **迷你条默认样式改为「渐变」**：未手动选过样式的安装默认得到封面主色渐变条（原默认毛玻璃）；已保存过样式的设备维持原选择，可在 设置 → 播放器 中切换

## 2026-09-18 — 迷你条默认样式改为渐变

### 设置

- **迷你条默认样式改为「渐变」**：未手动选过样式的安装默认得到封面主色渐变条（原默认毛玻璃）；已保存过样式的设备维持原选择，可在 设置 → 播放器 中切换

## 2026-09-17 (2) — 切歌取色连续性 + 面板透明度更名

### 播放器 / 皮肤

- **切歌取色连续**：切歌后新封面取色未到位前，播放页渐变/弹层/迷你条/albumTint 皮肤沿用上一首成功取到的颜色，背景不再闪回默认色；冷启动从未取到过任何颜色时回退莫奈主色（主题 primary）。新增「生效主色」provider（本次取色 ?? 进程级上一首记忆），取色写入走 ref.listen，不在 build 期改 state

### 设置

- **「卡片透明度」更名「面板透明度」**：盘点该滑杆实际作用面——玻璃卡片、顶栏、弹层、Tooltip/PopupMenu、提示气泡、列表头面板、浮层底、搜索框、瓷砖填充、播放页取色面板等，远不止卡片；更名使名称与感知作用面一致。未采用「背景透明度」：页面背景（氛围舞台/自定义图片）不随此滑杆变化，且与背景图片自身的不透明度设置语义冲突。滑杆与存储 key 不变，无迁移

## 2026-09-17 — 动效升级：氛围运动 + 播放页编排 + 微交互

### 全局氛围

- **皮肤舞台漂移**：液态玻璃双光斑、Material You 柔光球改为 Lissajous 轨迹缓慢游移（26s 一圈，相位错开不同步）；深空星点按各自相位呼吸闪烁（亮度+半径起伏）；RepaintBoundary 隔离循环重绘，页面内容零影响
- **省电降级**：氛围漂移/星点闪烁随省电模式完全静止（开关切换即时生效），自定义图片背景不受影响

### 播放器

- **级联入场**：打开播放页时顶栏→封面区→底部控制区按区间错峰上滑淡入（复用打开路由时间线，无新增控制器），关闭反向错峰退场
- **Tab 缩放联动**：推荐/歌曲/歌词左右滑时相邻页轻微缩小降透明，拖动每帧只重建变换壳
- **封面 punch-in + 扫光**：切歌封面淡入升级为 0.92 缩放落位，落位后一道斜向高光扫过（一次性自驱动）
- **唱片呼吸光晕**：黑胶/CD 背后新增封面主色光环，随播放呼吸、暂停冻结；OverflowBox 外溢绘制不挤动排版
- **播放键 morph**：播放/暂停图标缩放+旋转过渡切换，点击一圈白光扩散脉冲，播放中按钮外圈呼吸光环（省电静止）
- **状态弹跳**：收藏爱心、播放模式切换图标缩放过冲回落（新组件 PopOnChange）

### 列表

- **入场错峰**：列表/网格项入场淡入支持按序号 35ms/项错峰（封顶 25% 延迟），歌曲列表、流派网格、搜索结果同批进入依次浮现

## 2026-09-16 (2) — 服务器编辑

### 服务器管理

- **编辑服务器**：服务器卡片菜单新增「编辑」——可修改地址/端口/子路径/HTTPS/用户名/密码与备注名，全部字段从现有配置预填（密码从存储凭证回填）；保存时用新凭证完整走一次登录验证，失败原配置不动
- **编辑即刻生效**：编辑当前连接的服务器保存后 adapter 自动用新地址/凭证重建，无需重新添加；仅改备注名不发起网络请求
- **粘贴容错**：地址框粘贴完整 URL（含协议/子路径）自动拆解归一化，与登录页同款逻辑

## 2026-09-16 — 设置页顶栏统一 + 弹层透明度收口

### 外观

- **设置二级页顶栏不透明**：7 个设置子页（播放/音效/网络/存储/外观/播放器样式/系统）此前顶栏透明，列表滚动后标题与卡片文字叠加——统一去掉透明覆盖，回退主题默认背景（与服务器管理页一致）
- **播放页弹层透明度收口**：队列/歌曲操作/下载列表等弹层的封面取色底色不再硬编码 alpha 0.90 或全实色，统一乘全局「卡片透明度」系数——滑杆默认 100% 时观感不变，调低后弹层随之变透明；提示气泡/菜单/玻璃对话框此前已接同一系数

## 2026-09-15 (4) — 搜索历史与搜索页空态

### 搜索

- **搜索历史**：键盘提交搜索、点击结果条目（艺人/专辑/歌曲）记入历史（防抖中间词不入史），最多 15 条去重置顶，SharedPreferences 持久化跨启动保留
- **搜索页空态**：从纯空白改为搜索历史胶囊（点按回填立即重搜、尾部 × 单条删除、右上角一键清空）；无历史时显示搜索引导文案

## 2026-09-15 (3) — 播放页控制区上移 + 首页快照缓存

### 播放器

- **底部控制区留白加大**：全屏播放器底部控制行距屏幕下缘 12 → 24，全面屏手势区不再顶住按钮

### 首页

- **分区快照缓存（SWR）**：五个首页分区（最新专辑/每日推荐/最近播放/最常播放/随机专辑）改为「磁盘快照秒回 + 每次进入后台刷新」——旧实现是纯内存态，每次冷启动必然整圈转圈等网络；现在启动/切 Tab 先回放上次数据（无加载圈），后台静默重取，拉取失败保留旧数据不闪错误态
- **多服务器隔离**：快照 key 带服务器 id，切服回放各自缓存；网络请求期间切服的旧数据直接丢弃，不覆盖新服展示

## 2026-09-15 (2) — 遗留问题清零 + 后台下载队列

### 数据与同步正确性

- **快照写库失败不再丢弃成功拉取**：曲库/专辑全量 fetch 成功后，快照落库失败此前会退回旧数据甚至报错——现在写库失败只跳过快照补写，本次新鲜数据照常返回
- **Subsonic 全库枚举健壮化**：分页拉取尊重 `start` 偏移；防重复页（个别实现忽略 `songOffset` 恒返同一页，此前会循环拉满 limit 上限）；末页超出 limit 时截齐；快照 kind 升级 `songs_all_v2` 作废旧随机子集快照（libraryVersion 未变时旧数据会永久命中）
- **桌面小部件更新乱序消除**：等待封面拉取期间切歌/暂停会开启更新的同步，晚到的旧封面结果不再回退小部件状态（同步序号守卫）
- **小部件封面缓存键加服务器维度**：`albumId` 跨服同值（fnOS/Plex 数字 id）导致封面互串，缓存键改为 `serverId|albumId`
- **跑马灯测量对齐系统大字号**：TextPainter 测量补 `textScaler`（此前系统大字体下溢出判断/滚动距离失真），测量输入进缓存键，TextPainter 用毕释放

### 后台下载队列

- **下载不再阻塞等待**：歌曲/批量下载从「全程进度对话框」改为入列后台队列立即返回——顺序落盘、蜂窝门禁与去重保留，重复点击不再重复下载
- **下载列表弹层**：设置-存储与缓存新增「下载列表」入口（显示进行中任务数），弹层内查看 排队/进度/完成/失败，失败可重试、非运行任务可移除、可一键清已完成
- **切服中止旧任务**：任务起跑时校验服务器身份，排队期间切服的旧任务标记失败不再下载
- **列表刷新收口**：下载完成后统一在队列排空时 bump「本地音乐」合并展示，避免批量下载期间连续全量重读

### 车机

- **Android Auto 补 `playFromMediaId`**：车载语音/推荐位按媒体 id 点播走该回调，此前只实现 `playMediaItem` 导致「能浏览不能点播」

## 2026-09-15 — 播放时序与队列/下载跨服隔离修复

### 播放时序

- **`play()` await 语义修复**：just_audio 的 `play()` future 要到暂停/播完才 complete，此前 `await _player.play()` 使首次起播阻塞在加载流程内——断点续播判定（`position > 0` 早退）永远命中，长音频断点功能实际失效。现在装源完成后、起播前完成 seek，`play()` 内不再等待播放结束；冷启动恢复进度改为 `_pendingResumeMs` 一次性消费（进 `play()` 即取走），杜绝跨歌泄漏
- **断点回写竞态消除**：playingStream / positionStream 是种子流，绑定瞬间就会吐出 `false` / `Duration.zero`——此时落盘会把零进度写回，覆盖已存断点与恢复进度。playingStream 监听改为「真转假」才触发暂停落盘，`_tickPositionPersist` 加零进度守卫
- **切服取消播放旧流程**：`stop()` 递增 `_playGeneration` 并先清 currentSong 再停播放器，切服瞬间在途的装源/加载流程在检查点处作废，不再把旧服务器的音源装进新会话
- **交叉淡化代数认领**：`play()` 返回本次创建的播放代数，淡入循环用返回值（而非事后读 `_playGeneration`）判定归属——装源期间用户手动切歌时，淡化正确放弃并恢复音量

### 队列与下载跨服隔离

- **FM 会话失效校验**：`start()` / `refillIfNeeded()` 的抽歌是两次网络请求，期间切服务器/换队列/退出 FM 后旧批量照常写入会串台——现在写入前比对 队列签名（长度|首曲|尾曲|当前曲，List 引用相等不可用）+ 活跃服务器 + FM 激活态，不一致即作废
- **下载指纹加服务器维度**：指纹从 `sha256(songId)` 改为 `sha256(serverId|songId)` 前 16 位——fnOS/Plex 等数字 id 服务器间同名 id 的歌曲不再共用/互串下载文件。历史索引与文件懒迁移：旧指纹记录归属可确认（无归属或本服务器）时改写为新指纹，文件名不变无需重下；他服记录不认领，目录扫描兜底同样过归属校验
- **手动补传 serverId**：歌曲操作弹窗的批量下载此前未传 serverId（落库归属为空串），与指纹归属对齐
- **本地音乐已下载段按服过滤**：`loadDownloadedSongs` 只展示归属当前服务器（或无归属历史行）的下载，切服自动刷新，不再出现「能看不能离线播」的他服条目
...
## 2026-09-12 — 私人 FM + 听歌统计 + A2 签名脚手架

- **私人 FM（首页横幅入口）**：从曲库随机漫游——抽歌复用 `SongSort.random`（7 后端全支持），排除近 7 天听过的歌曲后灌入全局播放队列，队列将尽（当前之后 ≤3 首）自动补一批；支持 不喜欢跳过 / 收藏（乐观更新）/ 播放暂停 / 下一首；切服后抽歌自动跟随当前服务器
- **听歌统计页（资料库九宫格入口）**：概览四项（总播放 / 收听时长 / 听过歌曲 / 近 30 天）+ 最常听歌曲 Top 20 + 最爱歌手 Top 10，下拉刷新；数据来自本机播放记录、仅计当前服务器（切服自动刷新），与后端 playCount 无关（fnOS/AudioStation 等不回传播放数也能统计）
- **本地播放历史表（DB v6）**：新增 `play_history` 表按服务器记录播放（歌曲元数据快照，删除歌曲不影响历史），单曲听满 20 秒（短歌半长）记一次，2 万条滚动裁剪；`PlayHistoryService` 随 App 存活、与 Scrobble 同源监听播放位置，同时为 FM 去重供数
- **Android 正式签名脚手架（A2）**：`android/app/build.gradle.kts` 改为 `key.properties` 驱动——文件存在即用正式 keystore 签 release，缺失自动回退 debug 签名（CI/克隆构建永不破）；`key.properties` 与 keystore 均已 gitignore，凭据不入库

## 2026-09-12 — 播放页移除 A-B 循环按钮

- **底部控制行去掉「AB」按钮**：A-B 段落循环功能整体移除（按钮、状态 provider、位置监听一并清退），控制行变为 模式 / 上一首 / 播放 / 下一首 / 队列 五键对称布局

## 2026-09-12 — 设置页转场统一 fadeRoute

- **消除设置二级页进入割裂感**：设置主菜单推入二级页与「添加服务器」推入原本走系统 `MaterialPageRoute`（整页滑入，背景跟随滑动，与首页内二级页观感割裂），统一改为全应用规范 `fadeRoute`（背景淡入 + 内容淡入微上移），与登录页/服务器详情页的转场一致

## 2026-09-12 — 资料库歌单切换：点击标题直接切换

- **歌单区「我的歌单 / 全部歌单」标题文字可点**：标题与三角图标合并为一个点击区（命中区更大），点击标题即切换视图，三角图标保持原交互；视觉与切换逻辑不变
- 空态引导文案同步改为「点击标题查看全部歌单」
