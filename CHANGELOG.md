...
## 2026-09-08 — 迷你播放条贴底收口：去掉下方背景露条

- 用户反馈（附截图）：迷你条下方露出「背景底色 + 安全区」一条，来源是 SafeArea 底部内缩 + 16px 下边距 + 旧负偏移下压
- **贴底收口**：AppShell 去掉 SafeArea 包裹；三样式（毛玻璃/纯色/渐变）统一去掉 16px 下边距、底边直角贴到屏幕物理底边（顶部保留胶囊圆角），系统手势条安全区吸收为条内底部 padding——条体自身占据原露条区域，不再透出页面背景
- **偏移范围 -20~40 → 0~40**：贴底后负值（下压）只会把条推出屏幕外，失去意义；滑杆范围与提示文案同步，旧存档负值在读取处 clamp 到 0
- MiniPlayer 四处使用点（shell / song_info / music_library / detail）共用此组件，改动全量生效
