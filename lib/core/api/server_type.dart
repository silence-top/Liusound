...
enum ServerType {
  ...
  ServerAdapter createAdapter(ServerConfig config, Map<String, String> secrets) {
    return switch (this) {
      ServerType.navidrome =>
        NavidromeAdapter(config: config, secrets: secrets),
      ServerType.subsonic =>
        SubsonicAdapter(config: config, secrets: secrets),
      ServerType.jellyfin =>
        JellyfinAdapter(config: config, secrets: secrets),
      ServerType.emby =>
        EmbyAdapter(config: config, secrets: secrets),
      ServerType.plex =>
        PlexAdapter(config: config, secrets: secrets),
      ServerType.audioStation =>
        AudioStationAdapter(config: config, secrets: secrets),
    };
  }
...
class ServerConfig {
  ...
  factory ServerConfig.fromJson(Map<String, dynamic> json) => ServerConfig(
        ...
        meta: (json['meta'] as Map<String, dynamic>?)?.cast<String, String>() ??
            const {},
      );
}
